...
# from ADL_ReID.frameworks.training import loss
import torch
...
class Baseline(nn.Module):
    ...
    def forward(self, batched_inputs, class2cid=None, iter=None):
        ...
        if self.training:
            ...
            losses = self.losses(outputs, targets, camids, class2cid, iter,)
            #  patch_feature_list,
    ...
    def losses(self, outputs, gt_labels, camids, class2cid, iter, ):
        #patch_feature_list,patch_features_anchor,outp_class_anchor)
        """
        ...
        cls_outputs       = outputs['classification_results']
        pred_features     = outputs['features']
        # bn_features       = outputs['bn_feat']
        # feature_dict_bn   = outputs['feature_dict_bn']
        cam_scores         = outputs['cam_scores']
        pzx_scores         = outputs['pzx_scores']
        pzpx_scores         = outputs['pzpx_scores']
        # Log prediction accuracy
        log_accuracy(pred_class_logits, gt_labels)
        trainingDAL = False
        loss_dict = {}
        # loss_dict["id_loss"] = self.xent(cls_outputs, gt_labels)
        ...
        if iter %20 == 0 :
            state = False
            set_train_mode(self.heads, state)
            ...
            trainingDAL = not state
        if trainingDAL:
            loss_dict["MI"] = -torch.mean(torch.log(pzx_scores+0.0001)+torch.log(1-pzpx_scores+0.0001))
        else:
            MI =  -torch.mean(torch.log(pzx_scores+0.0001)+torch.log(1-pzpx_scores+0.0001))

            loss_dict["MI"] = torch.exp(-MI/50)
        ...
        camids = camids.cuda() 
        # loss_dict["camloss"] = self.xent(cam_scores, camids)
        ...
        return loss_dict
def set_train_mode(heads, state):
	#print(state)
	set_grads(heads.RFM, state)
	# set_grads(heads.base, state)
	#set_grads(model.module.bottleneck, state)
	set_grads(heads.cam_classifier, state)
	set_grads(heads.classifier, state)
	set_grads(heads.DAL, not state)
